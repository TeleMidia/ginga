...
NTPPlayer::NTPPlayer (GingaScreenID screenId, string mrl)
    : Player (screenId, mrl)
{
...
time_t
NTPPlayer::getTime (unsigned short *milliseconds)
{
          ...
          if (udp.receive (&recvBuffer) == NTP_BUFFER_SIZE)
            {
              ...
              utcTime.time
                  = mktime (valueStruct) + NTPPlayer::localTimezone ();
...
double
NTPPlayer::diffTime (timeb a, timeb b)
{
...
void
NTPPlayer::addTime (timeb *a, double seconds)
{
  unsigned short ms;
...
timeb *
NTPPlayer::tm2timeb (struct tm *time, unsigned short milliseconds)
{
  time_t t;
  timeb *tb;
  tb = new timeb ();
...
unsigned short
NTPPlayer::ntpFrac2ms (alt_u32 value)
{
  return (unsigned short)(((double)value) / 0x100000000 * 1000);
}

short
NTPPlayer::localTimezone ()
{
  struct tm when = { 0 };
...
double
NTPPlayer::elapsedTime (string &oldNclTime)
{
