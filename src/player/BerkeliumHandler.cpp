  ...
  if (isValid)
    {
      ...
      /*
                              clog << "BerkeliumHandler::~BerkeliumHandler
         deleting win" << endl;
                              bWindow->del();
...
void
BerkeliumHandler::setKeyHandler (bool handler)
{
  Thread::mutexLock (&sMutex);
  if (isValid)
    {
...
void
BerkeliumHandler::setContext (Context *context)
{
  Thread::mutexLock (&sMutex);
...
void
BerkeliumHandler::setWindow (std::auto_ptr<Window> window)
{
  Thread::mutexLock (&sMutex);
  bWindow = window;
  isValid = true;
...
void
BerkeliumHandler::getSize (int *w, int *h)
{
  *w = this->w;
  *h = this->h;
}
...
void
BerkeliumHandler::setBounds (int x, int y, int w, int h)
{
  Thread::mutexLock (&sMutex);
  xOffset = x;
  yOffset = y;
...
bool
BerkeliumHandler::userEventReceived (SDLInputEvent *userEvent)
{
...
bool
BerkeliumHandler::motionEventReceived (int x, int y, int z)
{
...
void
BerkeliumHandler::onAddressBarChanged (Window *win, URLString newURL)
{
  std::string x = "hi";
  x += newURL;
...
void
BerkeliumHandler::onStartLoading (Window *win, URLString newURL)
{
  ...
  win->insertCSS (WideString::point_to (str_css.c_str ()),
                  WideString::empty ());
...
void
BerkeliumHandler::onLoadingStateChanged (Window *win, bool isLoading)
{
...
void
BerkeliumHandler::onLoad (Window *win)
{
  wstring str_css (L"::-webkit-scrollbar { display: none; }");
  wstring str_js (L"document.body.style.overflow='hidden'");
...
void
BerkeliumHandler::onLoadError (Window *win, WideString error)
{
...
void
BerkeliumHandler::onResponsive (Window *win)
{
  clog << "BerkeliumHandler::onResponsive " << mURL << endl;
}

void
BerkeliumHandler::onUnresponsive (Window *win)
{
  clog << "BerkeliumHandler::onUnresponsive " << mURL << endl;
}

bool
BerkeliumHandler::mapOnPaintToTexture (
  ...
  if (dx != 0 || dy != 0)
    {
      ...
      // region
      Berkelium::Rect scrolled_shared_rect
          = scroll_rect.intersect (scrolled_rect);
      ...
      if (scrolled_shared_rect.width () > 0
          && scrolled_shared_rect.height () > 0)
        {
          ...
          Berkelium::Rect shared_rect
              = scrolled_shared_rect.translate (dx, dy);
  ...
  for (size_t i = 0; i < num_copy_rects; i++)
    {
      ...
      unsigned char *tmp_buffer
          = new unsigned char[wid * hig * kBytesPerPixel];

      for (int jj = 0; jj < hig; jj++)
        {
          memcpy (tmp_buffer + jj * wid * kBytesPerPixel,
                  bitmap_in
                              ...
                              int height)
{
  ...
  FILE *outfile;
  {
    ...
    os << string (g_get_tmp_dir ()) + "/bh_r_" << time (NULL) << "_"
       << (callCount++) << ".ppm";
    str = os.str ();
...
void
BerkeliumHandler::onCrashed (Window *win)
{
...
void
BerkeliumHandler::onPaintPluginTexture (Window *win, void *sourceGLTexture,
...
void
BerkeliumHandler::onWidgetDestroyed (Window *win, Widget *newWidget)
{
...
void
BerkeliumHandler::onWidgetPaint (Window *win, Widget *wid,
