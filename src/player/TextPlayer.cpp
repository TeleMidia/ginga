                   ...
                   Color *fontColor)
{
  ...
  if (font != 0)
    {
      dm->setSurfaceColor (s, fontColor->getR (), fontColor->getG (),
                           fontColor->getB (), fontColor->getAlpha ());

      width = dm->getProviderStringWidth (
...
TextPlayer::drawText (string text, short align)
{
  ...
  dm->setSurfaceColor (surface, fontColor->getR (), fontColor->getG (),
                       fontColor->getB (), fontColor->getAlpha ());
      ...
      if (textWidth > surWidth && aux.length () > 1)
        {
          ...
          dm->playProviderOver (font, surface, aux.c_str (), currentColumn,
                                currentLine, align);
