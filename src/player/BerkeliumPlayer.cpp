...
BBrowserFactory::BBrowserFactory () { Thread::mutexInit (&smutex, false); }
...
void
BBrowserFactory::start ()
{
  running = true;
}

void
BBrowserFactory::stop ()
{
  running = false;
}

bool
BBrowserFactory::isRunning ()
{
  return running;
}

bool
BBrowserFactory::hasBrowser ()
{
  bool hasIt;
...
bool
BBrowserFactory::hasRunningBrowser ()
{
  bool bRunning = false;
...
void
BBrowserFactory::createBrowser (BerkeliumHandler *bInfo)
{
  lockSet ();
  bMap[bInfo] = BPT_INIT;
  unlockSet ();
...
void
BBrowserFactory::stopBrowser (BerkeliumHandler *bInfo)
{
  lockSet ();
  bMap[bInfo] = BPT_STOP;
  unlockSet ();
...
void
BBrowserFactory::destroyBrowser (BerkeliumHandler *bInfo)
{
  lockSet ();
...
void
BBrowserFactory::initBrowser (BerkeliumHandler *bInfo)
{
  Context *context;
  string mrl;
  int w, h;
  ...
  bwin->insertCSS (WideString::point_to (str_css.c_str ()),
                   WideString::empty ());
...
void
BBrowserFactory::lockSet ()
{
  Thread::mutexLock (&smutex);
}

void
BBrowserFactory::unlockSet ()
{
  Thread::mutexUnlock (&smutex);
}

BerkeliumPlayer::BerkeliumPlayer (GingaScreenID myScreen, string mrl)
    : Player (myScreen, mrl)
{
...
void
BerkeliumPlayer::setNotifyContentUpdate (bool notify)
{
...
void
BerkeliumPlayer::setBounds (int x, int y, int w, int h)
{
...
BerkeliumPlayer::play ()
{
  ...
  iWin = G_DisplayManager->getSurfaceParentWindow (surface);
  if (iWin != 0)
    {
      G_DisplayManager->renderWindowFrom (myScreen, iWin, surface);
    }
  return Player::play ();
}

void
BerkeliumPlayer::stop ()
{
...
bool
BerkeliumPlayer::setKeyHandler (bool isHandler)
{
...
void *
BerkeliumPlayer::mainLoop (void *ptr)
{
  mainLoopDone = false;
