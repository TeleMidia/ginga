...
uint64_t
Stc::baseExtToStc (uint64_t base, uint64_t ext)
{
  return ((base * 300) + ext);
}

uint64_t
Stc::stcToBase (uint64_t stc)
{
  uint64_t value = 0;
  value = 1 << 16;
  value = value << 17;
...
uint64_t
Stc::stcToExt (uint64_t stc)
{
  return (((uint64_t)stc) % 300);
}

double
Stc::stcToSecond (uint64_t stc)
{
  double ret = stc;
...
double
Stc::baseToSecond (uint64_t base)
{
  double ret = base;
...
uint64_t
Stc::secondToStc (double seconds)
{
  return (uint64_t) (seconds * SYSTEM_CLOCK_FREQUENCY);
}

uint64_t
Stc::secondToBase (double seconds)
{
...
void
Stc::setReference (uint64_t base, uint64_t ext)
{
