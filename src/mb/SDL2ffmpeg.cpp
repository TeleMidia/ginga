...
SDL2ffmpeg::prepare ()
{
  ...
  if ((unsigned int)wantedSpec.channels > 0
    ...
    {
      SDL_BuildAudioCVT (&acvt, wantedSpec.format, wantedSpec.channels,
                         wantedSpec.freq, spec.format, spec.channels,
                         spec.freq);
    }
...
void
SDL2ffmpeg::getOriginalResolution (int *w, int *h)
{
  if (vs->video_st)
    {
...
void
SDL2ffmpeg::setTexture (SDL_Texture *texture)
{
  this->texture = texture;
}

bool
SDL2ffmpeg::hasTexture ()
{
  return (texture != NULL);
}

SDL_Texture *
SDL2ffmpeg::getTexture ()
{
  return texture;
}

bool
SDL2ffmpeg::hasPicture ()
{
  return hasPic;
}
...
bool
SDL2ffmpeg::getAudioSpec (SDL_AudioSpec *spec, int sample_rate,
                          uint8_t channels)
{
...
int64_t
SDL2ffmpeg::get_valid_channel_layout (int64_t channel_layout, int channels)
{
  if (channel_layout
...
int
SDL2ffmpeg::nts_packet_queue_put (PacketQueue *q, AVPacket *pkt)
{
  MyAVPacketList *pkt1;
...
int
SDL2ffmpeg::packet_queue_put (PacketQueue *q, AVPacket *pkt)
{
  int ret;
...
int
SDL2ffmpeg::packet_queue_put_nullpacket (PacketQueue *q, int stream_index)
{
...
void
SDL2ffmpeg::packet_queue_init (PacketQueue *q)
{
...
void
SDL2ffmpeg::packet_queue_flush (PacketQueue *q)
{
  MyAVPacketList *pkt;
  MyAVPacketList *pkt1;
...
void
SDL2ffmpeg::packet_queue_destroy (PacketQueue *q)
{
...
void
SDL2ffmpeg::packet_queue_abort (PacketQueue *q)
{
  SDL_LockMutex (q->mutex);
...
void
SDL2ffmpeg::packet_queue_start (PacketQueue *q)
{
  SDL_LockMutex (q->mutex);
  q->abort_request = 0;
...
int
SDL2ffmpeg::packet_queue_get (PacketQueue *q, AVPacket *pkt, int block,
                              int *serial)
{
  MyAVPacketList *pkt1;
  int ret;
...
void
SDL2ffmpeg::render_vp (VideoPicture *vp)
{
  assert (vp->src_frame != NULL);
  ...
  if (vp->tex)
    {
      ...
      if (SDL_LockTexture (vp->tex, NULL, (void **)&pict.data,
                           &pict.linesize[0])
          != 0)
        {
      ...
      if (!abortRequest)
        {
          if (vp->tex && vp->src_frame->data && vp->height > 0)
            {
  ...
  // TODO: find a better way to check if video_thread already did this frame
  // unref
  if (vp->src_frame)
    {
...
void
SDL2ffmpeg::set_clock_at (Clock *c, double pts, int serial, double time)
{
  c->pts = pts;
  c->last_updated = time;
...
void
SDL2ffmpeg::set_clock (Clock *c, double pts, int serial)
{
  double time = av_gettime_relative () / 1000000.0;
...
void
SDL2ffmpeg::set_clock_speed (Clock *c, double speed)
{
...
void
SDL2ffmpeg::init_clock (Clock *c, int *queue_serial)
{
  c->speed = 1.0;
  c->paused = 0;
...
void
SDL2ffmpeg::sync_clock_to_slave (Clock *c, Clock *slave)
{
...
int
SDL2ffmpeg::get_master_sync_type ()
{
...
/* seek in the stream */
void
SDL2ffmpeg::stream_seek (int64_t pos, int64_t rel, int seek_by_bytes)
{
  if (!vs->seek_req)
    {
      vs->seek_pos = pos;
...
void
SDL2ffmpeg::stream_toggle_pause ()
{
  if (vs->paused)
    {
      vs->frame_timer += av_gettime_relative () / 1000000.0
                         + vs->vidclk.pts_drift - vs->vidclk.pts;
  ...
  vs->paused = vs->audclk.paused = vs->vidclk.paused = vs->extclk.paused
      = !vs->paused;
}

void
SDL2ffmpeg::toggle_pause ()
{
  stream_toggle_pause ();
  vs->step = 0;
}
...
double
SDL2ffmpeg::compute_target_delay (double delay)
{
...
double
SDL2ffmpeg::vp_duration (VideoState *vs, VideoPicture *vp,
                         VideoPicture *nextvp)
{
  double duration;
  ...
  if (vp->serial == nextvp->serial)
    {
      ...
      if (isnan (duration) || duration <= 0
          || duration > vs->max_frame_duration)
        {
          return vp->duration;
        }
      else
...
{
  int ret = vs->pictq_rindex_shown;
  vs->pictq_rindex_shown = 0;
  return ret;
}

void
SDL2ffmpeg::pictq_next_picture ()
{
  if (!vs->pictq_rindex_shown)
    {
      vs->pictq_rindex_shown = 1;
      return;
    }
...
void
SDL2ffmpeg::update_video_pts (double pts, int64_t pos, int serial)
{
...
void
SDL2ffmpeg::video_refresh (void *opaque, double *remaining_time)
{
  ...
  if (!vs->paused && dec->get_master_sync_type () == AV_SYNC_EXTERNAL_CLOCK
      && vs->realtime)
    {
      ...
      if (dec->pictq_nb_remaining () == 0)
      ...
      else
        {
          ...
          if (redisplay)
            ...
            {
              delay = dec->compute_target_delay (last_duration);
            }
          ...
          if (time < vs->frame_timer + delay && !redisplay)
            {
              *remaining_time
                  = FFMIN (vs->frame_timer + delay - time, *remaining_time);
              return;
            }
...
int
SDL2ffmpeg::queue_picture (AVFrame *src_frame, double pts, double duration,
  ...
  SDL_CondWait(vs->pictq_cond, vs->pictq_mutex);
      ...
      if (vs->videoq.abort_request &&
                      SDL_PeepEvents(
                                      ...
                                      FF_ALLOC_EVENT,
                                      FF_ALLOC_EVENT) != 1) {
          while (!vp->allocated && !vs->abort_request) {
              SDL_CondWait(vs->pictq_cond, vs->pictq_mutex);
          }
      }
      SDL_UnlockMutex(vs->pictq_mutex);*/
...
int
SDL2ffmpeg::get_video_frame (AVFrame *frame, AVPacket *pkt, int *serial)
{
  int got_picture;
  ...
  if (avcodec_decode_video2 (vs->video_st->codec, frame, &got_picture, pkt)
      < 0)
    {
      return 0;
    }
  ...
  if (got_picture)
    {
      ...
      frame->sample_aspect_ratio
          = av_guess_sample_aspect_ratio (vs->ic, vs->video_st, frame);
          ...
          if (frame->pts != AV_NOPTS_VALUE)
            {
              ...
              if (!isnan (diff) && fabs (diff) < AV_NOSYNC_THRESHOLD
                  ...
                  && *serial == vs->vidclk.serial && vs->videoq.nb_packets)
                {
...
// AVFILTER begin
int
SDL2ffmpeg::configure_filtergraph (AVFilterGraph *graph,
                                     ...
                                     const char *vfilters, AVFrame *frame)
{
  ...
  int ret;
  AVFilterContext *filt_src = NULL, *filt_out = NULL, *filt_deint = NULL,
                  *last_filter = NULL;
  ...
  if ((ret = avfilter_graph_create_filter (
           &filt_src, avfilter_get_by_name ("buffer"), "sdl2ffmpeg",
  ...
  if (yadif != NULL)
    {
      if ((ret = avfilter_graph_create_filter (
               &filt_deint, yadif, "sdl2ffmpeg_yadif", "parity=-1:deint=1",
               NULL, graph))
          < 0)
        {
          clog << "SDL2ffmpeg::configure_video_filters Warning! ";
    ...
    {
      clog << "SDL2ffmpeg::configure_video_filters Warning! ";
      clog << "Can't deinterlace video (yadif filter not installed";
      clog << endl;
      last_filter = filt_out;
    }
      ...
      if (rotate_tag && *rotate_tag->value
          && strcmp (rotate_tag->value, "0"))
        {
          if (!strcmp (rotate_tag->value, "90"))
            ...
            {
              char rotate_buf[64];
              snprintf (rotate_buf, sizeof (rotate_buf), "%s*PI/180",
                        rotate_tag->value);
...
int
SDL2ffmpeg::configure_audio_filters (const char *afilters,
                                     int force_output_format)
{
  ...
  ret = avfilter_graph_create_filter (
      &filt_asrc, avfilter_get_by_name ("abuffer"), "sdl2ffmpeg_abuffer",
  ...
  ret = avfilter_graph_create_filter (
      &filt_asink, avfilter_get_by_name ("abuffersink"),
      "sdl2ffmpeg_abuffersink", NULL, NULL, vs->agraph);
...
int
SDL2ffmpeg::video_thread (void *arg)
{
  AVPacket pkt = { 0 };
  ...
  while (!dec->abortRequest)
    {
      ...
      ret = dec->queue_picture (frame, pts, duration,
 ...
 */
int
SDL2ffmpeg::synchronize_audio (int nb_samples)
{
  ...
  if (get_master_sync_type () != AV_SYNC_AUDIO_MASTER)
    {
      ...
      if (!isnan (diff) && fabs (diff) < AV_NOSYNC_THRESHOLD)
        {
          vs->audio_diff_cum
              = diff + vs->audio_diff_avg_coef * vs->audio_diff_cum;
          ...
          if (vs->audio_diff_avg_count < AUDIO_DIFF_AVG_NB)
          ...
          else
            {
              ...
              if (fabs (avg_diff) >= vs->audio_diff_threshold)
                {
                  wanted_nb_samples
                      = nb_samples + (int)(diff * vs->audio_src.freq);
                  ...
                  wanted_nb_samples
                      = FFMIN (FFMAX (wanted_nb_samples, min_nb_samples),
                               max_nb_samples);
                }
            }
        }
      ...
      while (pkt_temp->stream_index != -1 || vs->audio_buf_frames_pending)
        {
          ...
          if (!vs->audio_buf_frames_pending)
            {
              len1 = avcodec_decode_audio4 (dec, vs->frame, &got_frame,
                                            pkt_temp);
              if (len1 < 0)
                {
              ...
              if (vs->frame->pts != AV_NOPTS_VALUE)
                {
                  vs->frame->pts
                      = av_rescale_q (vs->frame->pts, dec->time_base, tb);
                }
              ...
              if (vs->frame->pts != AV_NOPTS_VALUE)
                {
                  vs->audio_frame_next_pts
                      = vs->frame->pts + vs->frame->nb_samples;
                }
              ...
              reconfigure
                  = cmp_audio_fmts (vs->audio_filter_src.fmt,
          ...
          if ((ret = av_buffersink_get_frame_flags (vs->out_audio_filter,
                                                    vs->frame, 0))
              < 0)
            {
          ...
          if (vs->swr_ctx)
            {
              ...
              if (wanted_nb_samples != vs->frame->nb_samples)
                {
                  if (swr_set_compensation (
                          vs->swr_ctx,
                          (wanted_nb_samples - vs->frame->nb_samples)
                              * vs->audio_tgt.freq / vs->frame->sample_rate,
                          wanted_nb_samples * vs->audio_tgt.freq
                              / vs->frame->sample_rate)
                      < 0)
                    {
                      clog << "SDL2ffmpeg::audio_decode_frame ";
              ...
              len2 = swr_convert (vs->swr_ctx, out, out_count, in,
                                  vs->frame->nb_samples);
              if (len2 < 0)
                {
                  clog << "SDL2ffmpeg::audio_decode_frame ";
...
SDL2ffmpeg::sdl_audio_callback (arg_unused (void *opaque), Uint8 *stream, int len)
{
  ...
  Thread::mutexLock (&aiMutex);
  audio_cb_time = av_gettime_relative ();
  ...
  while (i != aInstances.end ())
    {
      ...
      if (dec->status == ST_PLAYING && vs != NULL && vs->audio_stream >= 0
          && vs->audio_st != NULL)
        {
          currentLen = len;
          offset = 0;
          ...
          if (!isnan (vs->audio_clock))
            {
              dec->set_clock_at (
                  &vs->audclk, vs->audio_clock
                                   - (double)(2 * vs->audio_hw_buf_size
                                              + vs->audio_write_buf_size)
                                         / vs->audio_tgt.bytes_per_sec,
                  vs->audio_clock_serial, audio_cb_time / 1000000.0);
                        ...
                        struct AudioParams *audio_hw_params)
{
  ...
  wantedSpec.samples = SDL_AUDIO_MIN_BUFFER_SIZE;
      ...
      while (SDL_OpenAudio (&wantedSpec, &spec) < 0)
        {
          ...
          wanted_channel_layout
              = av_get_default_channel_layout ((int)wantedSpec.channels);
        }
  ...
  if (spec.channels != wantedSpec.channels)
    {
      wanted_channel_layout
          = av_get_default_channel_layout ((int)spec.channels);
...
int
SDL2ffmpeg::stream_component_open (int stream_index)
{
  ...
  if (stream_lowres || (codec->capabilities & CODEC_CAP_DR1))
    {
  ...
  opts = filter_codec_opts (NULL, avctx->codec_id, ic,
  ...
  switch (avctx->codec_type)
    ...
    case AVMEDIA_TYPE_AUDIO:
      ...
      if ((ret = audio_open (channel_layout, nb_channels, sample_rate,
                             &vs->audio_tgt))
          < 0)
        {
          return ret;
      ...
      vs->audio_diff_threshold
          = (double)(vs->audio_hw_buf_size) / vs->audio_tgt.bytes_per_sec;
...
void
SDL2ffmpeg::stream_component_close (int stream_index)
{
  AVFormatContext *ic = vs->ic;
  AVCodecContext *avctx;
...
int
SDL2ffmpeg::decode_interrupt_cb (void *ctx)
{
...
SDL2ffmpeg::read_init ()
{
  ...
  av_format_inject_global_side_data (vs->ic);
  ...
  if (seek_by_bytes < 0)
    {
      seek_by_bytes = !!(vs->ic->iformat->flags & AVFMT_TS_DISCONT)
                      && strcmp ("ogg", vs->ic->iformat->name);
    }

  vs->max_frame_duration
      = (vs->ic->iformat->flags & AVFMT_TS_DISCONT) ? 10.0 : 3600.0;
...
int
SDL2ffmpeg::read_thread (void *arg)
{
  SDL2ffmpeg *dec = (SDL2ffmpeg *)arg;
  VideoState *vs = dec->vs;
  ...
  while (!dec->abortRequest)
    {
      ...
      if (vs->paused
          && (!strcmp (vs->ic->iformat->name, "rtsp")
              || (vs->ic->pb && !strncmp (vs->filename, "mmsh:", 5))))
        {
      ...
      if (vs->seek_req)
        {
          ...
          ret = avformat_seek_file (vs->ic, -1, seek_min, seek_target,
                                    seek_max, vs->seek_flags);
...
void
SDL2ffmpeg::stream_cycle_channel (int codec_type)
{
  AVFormatContext *ic = vs->ic;
  int start_index = 0, stream_index = 0;
  AVStream *st;
  AVProgram *p = NULL;
  int nb_streams = vs->ic->nb_streams;
...
int
SDL2ffmpeg::lockmgr (void **mtx, enum AVLockOp op)
{
  switch (op)
    {
    case AV_LOCK_CREATE:
