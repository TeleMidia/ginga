...
void
InputManager::initializeInputIntervalTime ()
{
  string strVar;
  ifstream fis;
...
InputManager::release ()
{
  ...
  mbKeyCode = G_DisplayManager->fromGingaToMB (myScreen, CodeMap::KEY_QUIT);
  ie = G_DisplayManager->createInputEvent (myScreen, NULL, mbKeyCode);
...
void
InputManager::addMotionEventListener (IMotionEventListener *listener)
{
...
void
InputManager::performInputLockedActions ()
{
  LockedAction *action;
...
void
InputManager::performApplicationLockedActions ()
{
  LockedAction *action;
  IInputEventListener *listener;
...
InputManager::postInputEvent (int keyCode)
{
  ...
  mbKeyCode = G_DisplayManager->fromGingaToMB (myScreen, keyCode);
  ie = G_DisplayManager->createInputEvent (myScreen, NULL, mbKeyCode);
  postInputEvent (ie);
}
...
void
InputManager::setAxisValues (int x, int y, int z)
{
  this->currentXAxis = x;
  this->currentYAxis = y;
  this->currentZAxis = z;
...
int
InputManager::getCurrentXAxisValue ()
{
  return currentXAxis;
}

int
InputManager::getCurrentYAxisValue ()
{
  return currentYAxis;
}
...
InputManager::getEventBuffer ()
{
  if (eventBuffer == NULL)
    {
      eventBuffer= G_DisplayManager->createEventBuffer (myScreen);
      if (!running)
        {
          running = true;
          Thread::startThread ();
  ...
  while (running && eventBuffer != NULL)
    {
      ...
      while (inputEvent != NULL)
        {
          handleInputEvent (inputEvent, pLastCode, lastCode, pTimeStamp,
