...
BR_PUCRIO_TELEMIDIA_GINGA_CORE_DATAPROCESSING_CAROUSEL_BEGIN
	...
	int DsmccDownloadDataBlock::processDataBlock(
		...
		while (rval > 0 && fd != NULL) {
			...
			if (messageId == 0x1003) {
				...
				if (rval == (int)messageLength) {
					...
					clog << "DsmccDownloadDataBlock::processDataBlock Warning!!" << endl;
					return -2;
				}
	...
	unsigned int DsmccDownloadDataBlock::getModuleId() {
		return moduleId;
	}

	unsigned int DsmccDownloadDataBlock::getModuleVersion() {
		return moduleVersion;
	}

	void DsmccDownloadDataBlock::print() {
