...
char DsmccMpegDescriptor::addData(char* data, unsigned short length) {
...
int DsmccMpegDescriptor::process() {
	return 2;
}

int DsmccMpegDescriptor::updateStream() {
	unsigned int len;
...
unsigned int DsmccMpegDescriptor::calculateDescriptorSize() {
	return 2;
}

unsigned char DsmccMpegDescriptor::isConsolidated() {
	unsigned short len = descriptorLength + 2;
	return (len <= currentSize);
}

unsigned char DsmccMpegDescriptor::getDescriptorTag() {
	return descriptorTag;
}

unsigned char DsmccMpegDescriptor::getDescriptorLength() {
	return descriptorLength;
}

void DsmccMpegDescriptor::setDescriptorTag(unsigned char tag) {
	descriptorTag = tag;
}

int DsmccMpegDescriptor::getStreamSize() {
	return calculateDescriptorSize();
}

int DsmccMpegDescriptor::getStream(char** dataStream) {
...
DsmccMpegDescriptor* DsmccMpegDescriptor::getDescriptor(
...
vector<DsmccMpegDescriptor*>* DsmccMpegDescriptor::getDescriptors(
		vector<DsmccMpegDescriptor*>* descriptors, unsigned char Tag) {
...
int DsmccMpegDescriptor::getDescriptorsLength(vector<DsmccMpegDescriptor*>* descriptors) {
	int len = 0;
	vector<DsmccMpegDescriptor*>::iterator dit;
	dit = descriptors->begin();
	while (dit != descriptors->end()) {
