...
GINGA_DATAPROC_BEGIN
	...
	DsmccBiop::DsmccBiop(DsmccModule* module, DsmccObjectProcessor* processor) {
		...
		if (fileExists(module->getModuleFileName())) {
			...
			if (!moduleFd) {
				clog << "DsmccBiop::init - Cannot open file: " << module->getModuleFileName().c_str() << endl;
				throw 1;
			}
	...
	void DsmccBiop::createData(unsigned int dataSize) {
		assert(dataSize > 0);
	...
	string DsmccBiop::getStringFromData(unsigned int offset, unsigned int len) {
		string strData = "";
	...
	string DsmccBiop::getObjectKind() {
		return objectKind;
	}

	string DsmccBiop::getObjectInfo() {
		return objectInfo;
	}

	void DsmccBiop::abortProcess(string warningText) {
	...
	bool DsmccBiop::processMessageHeader() {
		int rval;
		unsigned int i;
	...
	int DsmccBiop::processMessageSubHeader() {
		int rval;
		unsigned int len;
		...
		if (hasMoreBiopMessage) {
			...
			if (isValidHdr) {
				if (processMessageSubHeader() < 0) {
					clog << "DsmccBiop::skipObject Warning! Invalid SubHeader" << endl;
					return -1;
				}
	...
	int DsmccBiop::processServiceGateway(unsigned int srgObjectKey) {
		unsigned int i, len;
	...
	DsmccBinding* DsmccBiop::processBinding() {
	...
	void DsmccBiop::processIor(DsmccBinding* binding) {
		DsmccIor* ior;
		unsigned int len, n1;
		string strField;
	...
	void DsmccBiop::processDirectory() {
		unsigned int i, len;
		DsmccBinding* binding;
		DsmccObject* carouselObject;
