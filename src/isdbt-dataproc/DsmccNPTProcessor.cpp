...
void DsmccNPTProcessor::setNptPrinter(bool nptPrinter) {
	this->nptPrinter = nptPrinter;
}

void DsmccNPTProcessor::clearTables() {
...
void DsmccNPTProcessor::clearUnusedTimebase() {
...
uint64_t DsmccNPTProcessor::getSTCValue() {
	uint64_t stcValue;
	int valueType;
...
bool DsmccNPTProcessor::addLoopListener(unsigned char cid, ITimeBaseListener* ltn) {
...
bool DsmccNPTProcessor::removeLoopListener(unsigned char cid, ITimeBaseListener* ltn) {
	map<unsigned char, set<ITimeBaseProvider*>*>::iterator i;
	set<ITimeBaseProvider*>::iterator j;

	clog << "DsmccNPTProcessor::removeLoopListener() - cid = " << (cid & 0xFF) << endl;
...
bool DsmccNPTProcessor::addTimeListener(
...
bool DsmccNPTProcessor::removeTimeListener(
...
bool DsmccNPTProcessor::addIdListener(ITimeBaseListener* ltn) {
...
bool DsmccNPTProcessor::removeIdListener(ITimeBaseListener* ltn) {
	set<ITimeBaseProvider*>::iterator i;
...
unsigned char DsmccNPTProcessor::getOccurringTimeBaseId() {
	return occurringTimeBaseId;
}

unsigned char DsmccNPTProcessor::getCurrentTimeBaseId() {
	map<unsigned char, DsmccTimeBaseClock*>::iterator i;
	DsmccTimeBaseClock* clk;
...
DsmccTimeBaseClock* DsmccNPTProcessor::getCurrentTimebase() {
	map<unsigned char, DsmccTimeBaseClock*>::iterator i;
	DsmccTimeBaseClock* clk;
...
void DsmccNPTProcessor::notifyLoopToTimeListeners() {
	map<unsigned char, set<ITimeBaseProvider*>*>::iterator i;
	set<ITimeBaseProvider*>::iterator j;
...
void DsmccNPTProcessor::notifyNaturalEndListeners(unsigned char cid, double nptValue) {
	map<unsigned char, set<ITimeBaseProvider*>*>::iterator i;
	set<ITimeBaseProvider*>::iterator j;
...
void DsmccNPTProcessor::notifyTimeListeners(unsigned char cid, double nptValue) {
	map<unsigned char, map<TimeControl*, set<ITimeBaseProvider*>*>*>::iterator i;
	map<TimeControl*, set<ITimeBaseProvider*>*>::iterator j;
	set<ITimeBaseProvider*>::iterator k;

	clog << "DsmccNPTProcessor::notifyTimeListeners " << "cid '" << (cid & 0xFF);
...
void DsmccNPTProcessor::notifyIdListeners(
...
DsmccTimeBaseClock* DsmccNPTProcessor::getTimeBaseClock(unsigned char cid) {
	map<unsigned char, DsmccTimeBaseClock*>::iterator i;
...
int DsmccNPTProcessor::updateTimeBase(DsmccTimeBaseClock* clk, DsmccNPTReference* npt) {
	...
	if (stcValue >= npt->getStcRef()) {
		...
		value = DsmccTimeBaseClock::convertToNpt(value,
...
double DsmccNPTProcessor::getCurrentTimeValue(unsigned char timeBaseId) {
	map<unsigned char, DsmccTimeBaseClock*>::iterator i;
	DsmccTimeBaseClock* clk;
...
void DsmccNPTProcessor::detectLoop() {
	DsmccTimeBaseClock* clk;
...
int DsmccNPTProcessor::decodeDescriptors(vector<DsmccMpegDescriptor*>* list) {
	...
	while (it != list->end()) {
		...
		if (desc->getDescriptorTag() == 0x01) {
			...
			clog << "DsmccNPTProcessor::decodeDescriptors unknown NPT descriptor ";
...
double DsmccNPTProcessor::getNPTValue(unsigned char contentId) {
	DsmccTimeBaseClock* clk;
...
char DsmccNPTProcessor::getNextNptValue(
...
bool DsmccNPTProcessor::processNptValues() {
	...
	while (restart) {
		...
		while (it != scheduledNpts.end()) {
			...
			if (getSTCValue() >= npt->getStcRef()) {
				...
				reScheduleIt = true;
				clog << "DsmccNPTProcessor::processNptValues - Executing scheduled NPT: "
