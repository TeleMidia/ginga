...
BR_PUCRIO_TELEMIDIA_GINGA_CORE_DATAPROCESSING_NCL_BEGIN
	...
	string NCLMetadata::getName() {
		return this->name;
	}

	double NCLMetadata::getTotalLength() {
		return this->totalLength;
	}

	void NCLMetadata::setTargetTotalLength(double targetSize) {
		this->targetTotalLen = targetSize;
	}

	bool NCLMetadata::isConsolidated() {
	...
	void NCLMetadata::setBaseUri(string uri) {
		this->baseUri = uri;
		this->mdUri   = SystemCompat::updatePath(baseUri + "/metadata.xml");
	}

	string NCLMetadata::getBaseUri() {
		return baseUri;
	}

	void NCLMetadata::setRootObject(INCLDataFile* rootObject) {
		if (updateTotalLength(rootObject)) {
			this->rootObject = rootObject;
		}
	}

	INCLDataFile* NCLMetadata::getRootObject() {
		return rootObject;
	}

	void NCLMetadata::addDataFile(INCLDataFile* dataFile) {
		updateTotalLength(dataFile);
		(*dataFiles)[dataFile->getId()] = dataFile;
	}

	INCLDataFile* NCLMetadata::getDataFile(int structureId) {
		map<int, INCLDataFile*>::iterator i;
	...
	map<int, INCLDataFile*>* NCLMetadata::getDataFiles() {
		return dataFiles;
	}

	bool NCLMetadata::updateTotalLength(INCLDataFile* dataFile) {
	...
	vector<StreamData*>* NCLMetadata::createNCLSections() {
		map<int, INCLDataFile*>::iterator i;
	...
	bool NCLMetadata::createMetadataFile() {
		remove(mdUri.c_str());
	...
	void NCLMetadata::closeMetadataFile() {
		mdFile->close();
		delete mdFile;
		mdFile = NULL;
	}

	void NCLMetadata::openMetadataElement() {
		*mdFile << "<metadata name=\"" << name << "\" size=\"";
		*mdFile << totalLength << "\">" << endl;
	}

	void NCLMetadata::closeMetadataElement() {
		*mdFile << "</metadata>" << endl;
	}

	void NCLMetadata::openBaseDataElement() {
		*mdFile << "  <baseData uri=\"" << baseUri << "\">" << endl;
	}

	void NCLMetadata::closeBaseDataElement() {
		*mdFile << "  </baseData>" << endl;
	}

	void NCLMetadata::writeRootElement(
	...
	void NCLMetadata::writeDataElement(
			string sId, string uri, string size, string componentTag) {
	...
	void NCLMetadata::copyContent(string uri, char* stream, int fileSize) {
		FILE* fd;
		int bytes;
		string absUri;
	...
	StreamData* NCLMetadata::createStreamData(
			int structId,
			int structType,
			string uri,
			int fileSize) {
	...
	int NCLMetadata::getFileSize(string uri) {
		FILE* fd;
		int bytes = 0;
