...
BR_PUCRIO_TELEMIDIA_GINGA_CORE_DATAPROCESSING_CAROUSEL_BEGIN
	...
	DsmccIor* DsmccDownloadServerInitiate::getServiceGatewayIor() {
		return this->srgIor;
	}

	int DsmccDownloadServerInitiate::processMessage(DsmccMessageHeader* message) {
		FILE* fd;
		int rval;
		...
		if (fd >= 0) {
			...
			clog << "DsmccDownloadServerInitiate::DsmccDownloadServerInitiate - Message header error: could not open file ";
	...
	int DsmccDownloadServerInitiate::processIor() {
		unsigned int len;
		char* field;
