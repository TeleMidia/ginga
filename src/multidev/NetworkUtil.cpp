...
bool
isValidRecvFrame (int recvFrom, int myIP, char *frame)
{
  // ATS: if the recvFrom = myIP the frame is sent along, in order to be
  // validated based on its destClass
          ...
          if (frameId == i->second)
            {
              ...
              if (recvFrom == myIP)
                {
                  clog << "NetworkUtil::isValidRecvFrame():isControlFrame("
                          "frameType) and (recvFrom == myIP)"
                       << endl;
...
char
getFrameId (char *frame)
{
  return frame[0];
}

int
getFrameType (char *frame)
{
  return (((unsigned char)frame[6]) & 0xFF);
}

unsigned int
getUIntFromStream (const char *stream)
{
...
char *
getStreamFromUInt (unsigned int uint)
{
  char *stream;
