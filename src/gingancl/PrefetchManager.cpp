...
void
PrefetchManager::releaseContents ()
{
  ...
  if (isRemoteDoc)
    {
      ...
      remoteRoot = remoteDocUri.substr (
          0, remoteDocUri.find_last_of (SystemCompat::getIUriD ()));
      localRoot = getLocalRoot (remoteDocUri);
    }
  else
    { // document is local => create local src relative to localDoc
      localRoot = localDocUri.substr (
          0, localDocUri.find_last_of (SystemCompat::getIUriD ()));
    }
      ...
      if (isRemoteDoc)
        {
          if (src.find (remoteRoot) == std::string::npos)
            ...
            {
              relativeSrc
                  = src.substr (remoteRoot.length () + 1,
              ...
              if (src.find (SystemCompat::getIUriD ()) != std::string::npos)
                {
                  relativeSrc = src.substr (
                      src.find_last_of (SystemCompat::getIUriD ()) + 1,
                      src.length ()
                          - (src.find_last_of (SystemCompat::getIUriD ())
                             + 1));
                }
              else
                {
...
void
PrefetchManager::setSynchPrefetch (bool synch)
{
  this->synch = synch;
}
...
void
PrefetchManager::getScheduledContents ()
{
  set<string>::iterator i, j;
