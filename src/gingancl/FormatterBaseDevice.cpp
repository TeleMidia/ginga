                          ...
                          useMulticast, srvPort)
{
  ...
  deviceLayout->addDevice ("systemScreen(1)", 0, 0,
                           DV_QVGA_WIDTH, DV_QVGA_HEIGHT);

  serialized = dm->createWindow (myScreen, 0, 0,
                                 DV_QVGA_WIDTH, DV_QVGA_HEIGHT, -1.0);
...
bool FormatterBaseDevice::newDeviceConnected(int newDevClass,int w,int h)
{
  return FormatterMultiDevice::newDeviceConnected(newDevClass, w, h);
}

bool FormatterBaseDevice::receiveRemoteEvent(int remoteDevClass,
                                             int eventType,
                                             string eventContent)
{
