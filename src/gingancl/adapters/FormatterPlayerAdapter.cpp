...
void
FormatterPlayerAdapter::setAdapterManager (void *manager)
{

  this->manager = manager;
  this->myScreen
      = ((PlayerAdapterManager *)manager)->getNclPlayerData ()->screenId;
  if (dm == NULL)
    {
...
bool
FormatterPlayerAdapter::instanceOf (string s)
{
  if (!typeSet.empty ())
    {
...
void
FormatterPlayerAdapter::setOutputWindow (GingaWindowID windowId)
{
  if (player != NULL)
    {
...
int
FormatterPlayerAdapter::getObjectDevice ()
{
  CascadingDescriptor *descriptor;
...
bool
FormatterPlayerAdapter::hasPrepared ()
{
  bool presented;
  FormatterEvent *mEv;
  short st;
...
bool
FormatterPlayerAdapter::setKeyHandler (bool isHandler)
{
  if (player == NULL)
    {
      g_usleep (100000);
...
double
FormatterPlayerAdapter::prepareProperties (ExecutionObject *obj)
{
  CascadingDescriptor *descriptor;
  ...
  if (transpValue == -1
      && descriptor->getParameterValue ("transparency") == "")
    {
      ...
      transpValue = ((PlayerAdapterManager *)manager)
                        ->getNclPlayerData ()
                        ->transparency;
...
void
FormatterPlayerAdapter::updatePlayerProperties (ExecutionObject *obj)
{
  CascadingDescriptor *descriptor;
  string value;
...
bool
FormatterPlayerAdapter::prepare (ExecutionObject *object,
...
void
FormatterPlayerAdapter::prepare ()
{
  CascadingDescriptor *descriptor;
  LayoutRegion *region;
...
void
FormatterPlayerAdapter::prepareScope (double offset)
{
...
double
FormatterPlayerAdapter::getOutTransDur ()
{
  CascadingDescriptor *descriptor;
...
double
FormatterPlayerAdapter::getOutTransTime ()
{
  return outTransTime;
}

void
FormatterPlayerAdapter::checkAnchorMonitor ()
{
...
void
FormatterPlayerAdapter::printAction (string action, ExecutionObject *object)
{
...
void
FormatterPlayerAdapter::printAction (string command)
{
...
void
FormatterPlayerAdapter::naturalEnd ()
{
  bool freeze;
...
bool
FormatterPlayerAdapter::checkRepeat (PresentationEvent *event)
{
...
bool
FormatterPlayerAdapter::unprepare ()
{
...
bool
FormatterPlayerAdapter::setPropertyValue (AttributionEvent *event,
                                          string value)
{
  ...
  if (propName == "visible")
  ...
  else
    {
      if (this->instanceOf ("ApplicationPlayerAdapter"))
        {
          if (!((ApplicationPlayerAdapter *)this)
...
void
FormatterPlayerAdapter::setPropertyValue (string name, string value)
{
  if (player != NULL)
    {
...
string
FormatterPlayerAdapter::getPropertyValue (void *event)
{
  string value = "";
  string name;
...
string
FormatterPlayerAdapter::getPropertyValue (string name)
{
  string value = "";
...
double
FormatterPlayerAdapter::getObjectExpectedDuration ()
{
  return IntervalAnchor::OBJECT_DURATION;
}

void
FormatterPlayerAdapter::updateObjectExpectedDuration ()
{
...
double
FormatterPlayerAdapter::getMediaTime ()
{
  if (player != NULL)
    {
...
IPlayer *
FormatterPlayerAdapter::getPlayer ()
{
  return player;
}

void
FormatterPlayerAdapter::setTimeBasePlayer (
...
void
FormatterPlayerAdapter::updateStatus (short code, string parameter, short type,
                                      string value)
{
...
bool
FormatterPlayerAdapter::userEventReceived (SDLInputEvent *ev)
{
  int keyCode;
...
void
FormatterPlayerAdapter::setVisible (bool visible)
{
  CascadingDescriptor *descriptor;
...
void
FormatterPlayerAdapter::timeShift (string direction)
{
  if (player == NULL)
    {
      return;
...
bool
FormatterPlayerAdapter::lockObject ()
{
  if (isLocked)
    {
      return false;
...
bool
FormatterPlayerAdapter::unlockObject ()
{
  if (!isLocked)
    {
      return false;
