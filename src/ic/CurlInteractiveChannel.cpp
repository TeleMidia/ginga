...
bool
CurlInteractiveChannel::hasConnection ()
{
  CURLcode res;
  ...
  if (localUri.find (SystemCompat::getIUriD ()) != std::string::npos)
    {
      localPath = localUri.substr (
          0, localUri.find_last_of (SystemCompat::getIUriD ()) + 1);
...
void
CurlInteractiveChannel::setTarget (FILE *fd)
{
  this->fd = fd;
}
...
bool
CurlInteractiveChannel::performUrl ()
{
  long respCode;
  CURLcode res;
  bool success = false;
...
bool
CurlInteractiveChannel::releaseUrl ()
{
...
bool
CurlInteractiveChannel::positiveResponse (long *respCode)
{
...
FILE *
CurlInteractiveChannel::getLocalFileDescriptor ()
{
  return fd;
}

IInteractiveChannelListener *
CurlInteractiveChannel::getListener ()
{
  return listener;
}
