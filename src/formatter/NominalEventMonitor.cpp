...
NominalEventMonitor::prepareNptTransitionsEvents ()
{
  ...
  executionObject->updateTransitionTable (
      timeBaseProvider->getCurrentTimeValue (timeBaseId), NULL,
      ContentAnchor::CAT_NPT);
...
void
NominalEventMonitor::loopDetected ()
{
                                   ...
                                   double timeValue)
{
  ...
  executionObject->updateTransitionTable (timeValue, adapter->getPlayer (),
                                          ContentAnchor::CAT_NPT);
}

void
NominalEventMonitor::startMonitor ()
{
  ...
  if (!running && !isDeleting)
    {
      timeSet
          = executionObject->getTransitionsValues (ContentAnchor::CAT_TIME);
...
void
NominalEventMonitor::pauseMonitor ()
{
...
{
  if (!isinf (expectedSleepTime) && paused)
    {
      paused = false;
      unlockConditionSatisfied ();
    }
...
void
NominalEventMonitor::stopMonitor ()
{
  stopped = true;
