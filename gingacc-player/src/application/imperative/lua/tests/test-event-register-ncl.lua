--[[ test-event-register-ncl.lua -- Check event.register with 'ncl' class.
...

----------------------------------------------------------------------------
-- event.register (pos, f, class, type, label, action) -> status, errmsg
----------------------------------------------------------------------------
...
assert (qsz () == 1)
assert (qf (1) == f and qcmp (1, {class='ncl'}))

assert (event.register (g, 'ncl', 'presentation'))        -- Q=<f,g>
assert (qsz () == 2)                                      --      ^
assert (qf (2) == g and qcmp (2, {class='ncl', type='presentation'}))

assert (event.register (1, h, 'ncl', 'selection'))        -- Q=<h,f,g>
assert (qsz () == 3)                                      --    ^
assert (qf (1) == h and qcmp (1, {class='ncl', type='selection'}))

-- Insert at position 3.
assert (event.register (3, f, 'ncl', nil, nil, 'start'))  -- Q=<h,f,f,g>
assert (qsz () == 4)                                      --        ^
assert (qf (3) == f and qcmp (3, {class='ncl', action='start'}))

assert (event.register (g, 'ncl', 'presentation', 'x'))   -- Q=<h,f,f,g,g>
assert (qsz () == 5)                                      --            ^
assert (qf (5) == g)
assert (qcmp (5, {class='ncl', type='presentation', label='x'}))

assert (event.register (f, 'ncl', 'presentation', 'y', 'abort'))
assert (qsz () == 6)                                      -- Q=<h,f,f,g,g,f>
assert (qf (6) == f)                                      --              ^
assert (qcmp (6, {class='ncl', type='presentation', label='y', action='abort'}))

-- Insert at position 2.
assert (event.register (2, h, 'ncl', 'attribution', 'x')) -- Q=<h,h,f,f,g,g,f>
assert (qsz () == 7)                                      --      ^
assert (qf (2) == h)
assert (qcmp (2, {class='ncl', type='attribution', name='x'}))

-- Warning: ignoring extra arguments.
assert (event.register (g, 'ncl', 'attribution', 'y', 'stop', 'z'))
assert (qsz () == 8)                                      -- Q=<h,h,f,f,g,g,f,g>
assert (qf (8) == g)                                      --                  ^
assert (qcmp (8, {class='ncl', type='attribution', name='y', action='stop'}))

-- Check whole queue.
assert (qf (1) == h and qcmp (1, {class='ncl', type='selection'}))
assert (qf (2) == h and qcmp (2, {class='ncl', type='attribution', name='x'}))
assert (qf (3) == f and qcmp (3, {class='ncl'}))
assert (qf (4) == f and qcmp (4, {class='ncl', action='start'}))
assert (qf (5) == g and qcmp (5, {class='ncl', type='presentation'}))
assert (qf (6) == g and qcmp (6, {class='ncl', type='presentation', label='x'}))
assert (qf (7) == f and qcmp (7, {class='ncl', type='presentation', label='y', action='abort'}))
assert (qf (8) == g and qcmp (8, {class='ncl', type='attribution', name='y', action='stop'}))
