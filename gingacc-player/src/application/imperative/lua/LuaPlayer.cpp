...
// Traverses the list of active NCLua state cycling and handling the events
// generated by each entry.  This thread is created when the first NCLua
// state is allocated, and it is destroyed whenever the last NCLua state is
// destroyed.
...
void *LuaPlayer::nc_update_thread (void *data)
{
     while (true)
     {
          ...
          for (i = lst.begin (); i != lst.end (); i++)
          {
               ...
               player = (LuaPlayer *) nclua_get_user_data (*i, NULL);
               player->lock ();
...
// Inserts state NC into update list.
// If this is the first state, creates both the list and the update thread.

void LuaPlayer::nc_update_insert (nclua_t *nc)
{
     ...
     nc_update_list->push_back (nc);
     MUTEX_UNLOCK (&nc_update_mutex);
}

// Removes state NC from update list.
// If this is the last state, destroys both the list and the update thread.

void LuaPlayer::nc_update_remove (nclua_t *nc)
{
     MUTEX_LOCK (&nc_update_mutex);
