...
<body>\n\
  ...
  <switch id='s1'>\n\
    ...
    Switch *swtch = cast (Switch *, body->getChildById ("s1"));
    g_assert_nonnull (swtch);
    Event *swtch_lambda = swtch->getLambda ();
    g_assert_nonnull (swtch_lambda);

    Media *m1 = cast (Media *, swtch->getChildById ("m1"));
    g_assert_nonnull (m1);
    Event *m1_lambda = m1->getLambda ();
    g_assert_nonnull (m1_lambda);

    Media *m2 = cast (Media *, swtch->getChildById ("m2"));
    g_assert_nonnull (m2);
    ...
    g_assert (swtch_lambda->getState () == Event::SLEEPING);
    ...
    fmt->sendTick (0, 0, 0);
    g_assert (body_lambda->getState () == Event::OCCURRING);
    g_assert (swtch_lambda->getState () == Event::OCCURRING);
    g_assert (m1_lambda->getState () == Event::OCCURRING);
    g_assert (m2_lambda->getState () == Event::SLEEPING);

    // after advance time, s1@lambda and its anchors
    ...
    g_assert_false (swtch_lambda->transition (Event::START));

    // after start, s1@lambda and its anchors
    // are OCCURRING, and its properties are SLEEPING
    g_assert (body_lambda->getState () == Event::OCCURRING);
    g_assert (swtch_lambda->getState () == Event::OCCURRING);
    g_assert (m1_lambda->getState () == Event::OCCURRING);
