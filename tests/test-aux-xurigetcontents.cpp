...
#define AUTHORS_FILE_LOCAL_RELATIVE_PATH TOP_SRCDIR "/AUTHORS"
...
main (void)
{
  ...
  // invalid uri.
  {
    ...
    g_assert (empty
              == "empty"); // doesn't change the string passed as param.
  }
  ...
  // relative path. (also invalid uri.)
  {
    // It must return false and not change the contents string.
    string empty = "empty";
    g_assert (xurigetcontents (AUTHORS_FILE_LOCAL_RELATIVE_PATH, empty)
              == false);
    g_assert (empty
              == "empty"); // doesn't change the string passed as param.
  }
