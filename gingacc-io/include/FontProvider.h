...
#ifndef FONTPROVIDER_H_
#define FONTPROVIDER_H_
...
namespace core {
namespace io {
	class FontProvider : IOContainer {
		protected:
			void* renderer;
		...
		public:
			...
			FontProvider(char* fontUri, int heightInPixel);
			virtual ~FontProvider();
			void* getContent();
			int getMaxAdvance();
			int getStringWidth(char* text, int textLength=0);
			int getHeight();
			void playOver(
					void* surface,
					char* text,
					int x=2, int y=0, short align=A_TOP_LEFT);
	};
}
}
}
